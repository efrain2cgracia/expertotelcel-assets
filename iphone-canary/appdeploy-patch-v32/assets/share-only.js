(() => {
  ...
  const appId = window.__APPDEPLOY_APP_ID || 'iphone-experto-telcel-zvrqk3';
  const managedHost = Boolean(window.__APPDEPLOY_APP_ID) ||
    window.location.hostname === 'iphone.expertotelcel.com' ||
    window.location.hostname.endsWith('.appdeploy.ai');
  const officialOverlayIsVisible = async () => {
    if (!managedHost) return false;
    try {
      const response = await fetch('https://api-v2.appdeploy.ai/p', {
        method: 'POST',
      ...
      return data?.show_overlay !== false;
    } catch (_) {
      return true;
    }
  };
  ...
  const start = async () => {
    const overlayVisible = await officialOverlayIsVisible();
    const showNativeShare = !overlayVisible;
    button.hidden = !showNativeShare;
    button.classList.toggle('et-share-visible', showNativeShare);
    document.body?.classList.toggle('et-native-share-active', showNativeShare);
    reconcile();
